<?php echo $header; ?>
    ...
    <div class="d-flex mt-3 mb-3">
        <div class="me-auto p-2"><h3><?php echo __('posts.posts'); ?></h3></div>
        <div class="p-2">
            <div class="input-group border-03">
                <select onchange="location = this.value;" class=" form-select">
                    <?php foreach ($categories as $cat): ?>
                        <option value="<?php echo Uri::to('admin/posts/category/' . $cat->slug); ?>" <?php if (isset($category) and $category->id == $cat->id): ?> selected<?php endif; ?>><?php echo $cat->title; ?></option>
                    <?php endforeach; ?>
                </select>
        ...
        <?php if ($posts->count): ?>
        <div class="p-2">
                <?php echo Html::link('admin/posts/add', '<i class="bi bi-plus-lg"></i>', [
                    'class' => 'btn btn-outline-secondary',
                    'data-bs-toggle' => 'tooltip',
                    'data-bs-title' => __('posts.create_post')
                ]); ?>
        </div>
        <?php endif; ?>
    </div>
                ...
                <div class="pl-2 col-md-11 col-sm-11 col-11 py-1">
                    ...
                    <p class="text-secondary mt-1 mb-0">
                        <?php if ($article->status == 'published'): ?>
                            <span><?php echo __('posts.posted'); ?><?php echo Date::format($article->created); ?> </span> <?php else: ?>
                        <span class="<?php echo $article->status; ?>"> <?php echo __('global.' . $article->status); ?></span>
                        <?php endif; ?><!----><span
                                class="d-none d-md-inline">― <?php echo __('posts.updated'); ?> <?php echo Date::format($article->updated); ?></span>
                    </p>
                </div>
